...
### Changed — 히스토리 "기간별 변동" 5기간으로 재구성
1일 전 대비는 히스토리 상단 요약 카드와 중복이라 제거했고, 대신 **1주일 전**을 추가했습니다. 이제 기간별 변동은 1주일/1달/3달/6달/1년 5개 구간으로 순자산 추세를 보여줍니다. 각 기간은 해당 시점의 스냅샷이 없으면 행 자체가 숨겨집니다.

### Changed — 대시보드에서 부채 목록을 자산 페이지로 이동
대시보드는 "요약·차트·알림"에 집중하도록 부채 목록 카드를 제거했습니다. 부채는 이제 `/assets` 페이지 하단에서 자산과 함께 보입니다. 소유자 필터(전체/나/배우자/공동)도 부채에 동일하게 적용됩니다. 대시보드 상단 Net Worth 헤더의 "총 부채" 요약 숫자는 그대로 유지.

- `/assets`: 부채 섹션 + 합계 표시. 자산과 부채 모두 0개일 때만 empty 뷰.
- 페이지 제목은 "자산 목록" 유지 (네비 탭 "자산"과 일관).
